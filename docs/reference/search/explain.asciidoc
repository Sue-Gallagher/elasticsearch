...
The explain api computes a score explanation for a query and a specific document, letting you know whether a document matches or doesn't match a specific query.
...
There is a simpler way of specifying the query using the `q` parameter. The specified `q` parameter value is parsed the same way as the `query_string` query. The following example shows the same query as shown above using the `q` parameter:
...
`_source`::
    ...
    retrieve part of the document using `_source_include` & `_source_exclude` (see <<get-source-filtering,Get API>> for more details)

`stored_fields`::
    Allows to control which stored fields to return as part of the explained document.

`routing`::
    Controls the routing when routing was used during indexing.
