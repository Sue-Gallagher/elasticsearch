...
`index.query.default_field` index settings, which in turn defaults to * (all searchable fields).
...
`analyzer`::
The search on each field is performed independantly using the individual field's default analyzer.
