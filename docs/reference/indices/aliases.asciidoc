...
APIs in Elasticsearch accept an index name when working against a specific index, or several indices when applicable. The index aliases API allows aliasing an index with a name. All APIs automatically convert the alias name to the actual index name. An alias can also be mapped to more than one index, and when specifying it, the alias automatically expands to the aliased indices. An alias can also be associated with a filter that will automatically be applied when searching and routing values. 

NOTE: An alias cannot have the same name as an index.

This example associates the alias `alias1` with index `test1`:
